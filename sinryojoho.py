...
if selector=="診療情報提供書":
    ...
    horizontal=True, key="atesaki")
    col1, col2, col3 = st.columns(3)
    with col3:
        if atesaki == "かかりつけ医":
            kankei = st.radio("当院との関係性", ("関係性あり", "関係性なし", "施設/訪問医あて"), 
            horizontal=True, key="kankei")
    ...
    shohou_kaisi = st.checkbox("当院で初回処方を開始した場合にはチェックしてください", key="shohou_kaisi")
    if shohou_kaisi == True:
        col1, col2, col3 = st.columns(3)
        with col3:
            zensoku = st.checkbox("気管支喘息はありますか", key="zensoku")
    ...
    horizontal=True, key="shohou")
    if shohou == "切り替え":
        col1, col2= st.columns(2)
        with col2:
            kirikae_why = st.radio("切り替えの理由は何ですか",
            ("消化器症状", "焦燥", "徐脈", "その他"), key="kirikae_why", horizontal=True)

            if kirikae_why == "消化器症状":
                kirikae_riyu = "食欲不振・下痢などの消化器症状がみられ"
            if kirikae_why == "焦燥":
                kirikae_riyu = "焦燥や不安感が懸念され"
            if kirikae_why == "徐脈":
                kirikae_riyu = "自覚症状はないものの徐脈傾向にあり"
            if kirikae_why == "その他":
                kirikae_riyu = st.text_input("理由を教えてください", placeholder="例：パーキンソニズムの出現があり", key="kirikae_sonota")
    ...
    if kaisuu != "1":
        ...
        if hantei == True:
            ...
            # （抗認知症薬同量処方依頼）
            if shohou == "初回":
                kaishaku = f"神経心理検査では前回と比較して、ADAS曲線のnatural course（抗認知症薬を服用していない群）を上回っており、{naifuku_genkou}の効果がみられていると愚考いたします。ご本人ご家族の希望があり、今後は貴院にて抗認知症薬のご処方をご検討いただけますと幸甚です。大変お忙しいところに、また誠に厚かましいお願いとなり恐縮ではございますが、どうかご高配のほど何卒よろしくお願い申し上げます。"
            # （他院にてすでに処方開始されていた場合）
            if shohou == "継続":
                kaishaku = f"神経心理検査では前回と比較して、ADAS曲線のnatural course（抗認知症薬を服用していない群）を上回っており、{naifuku_genkou}の効果がみられていると愚考いたします。差し出がましく誠に恐縮ですが、先生にすでにご処方いただいております抗認知症薬の継続処方をご検討いただけますと幸甚です。大変お忙しいところに、また誠に厚かましいお願いとなり恐縮ではございますが、どうかご高配のほど何卒よろしくお願い申し上げます。"
            # （副作用のため切り替え）
            if shohou == "切り替え":
                kaishaku = f"神経心理検査では前回と比較して、ADAS曲線のnatural course（抗認知症薬を服用していない群）を上回っております。しかしながら{kirikae_riyu}、アリセプトの継続はリスクが高まると考え、例えばメマリーに切り替え、漸増処方することをご検討いただけますか（5→10→15→20mg）。大変差し出がましく、またお手数をおかけして申し訳ありませんが、どうかご高配のほど何卒よろしくお願い申し上げます。"
...
if selector=="精査依頼":
    ...
    irai_byomei = st.radio("精査を依頼する疾患名を入力してください",
    ('レビー小体型認知症', '正常圧水頭症', '脳腫瘍', '脳動脈瘤', '精神科入院依頼', '進行性核上性麻痺'), 
    horizontal=True, key="irai_byomei")
...
if selector=="診療情報提供書 木之下先生ver":
    ...
    #####木之下先生修正
    atesaki_tegami += f"大変お忙しい中恐縮です。どうかよろしくご査収賜れれば幸甚です。"
    ...
    if kaisuu != "1":
        ...
        if hantei == True:
            ...
            # （抗認知症薬同量処方依頼）
            if shohou == "初回":
                kaishaku = f"神経心理検査では前回と比較して、ADAS曲線のnatural course（抗認知症薬を服用していない群）を上回っており、{naifuku_genkou}の効果がみられていると愚考いたします。ご本人ご家族の希望があり、今後は貴院にて抗認知症薬のご処方をご検討いただけますと幸甚です。大変お忙しいところに、また誠に厚かましいお願いとなり恐縮ではございますが、どうか引き続きご高診賜りますようお願い申し上げます。"
            # （他院にてすでに処方開始されていた場合）
            if shohou == "継続":
                kaishaku = f"神経心理検査では前回と比較して、ADAS曲線のnatural course（抗認知症薬を服用していない群）を上回っており、{naifuku_genkou}の効果がみられていると愚考いたします。差し出がましく誠に恐縮ですが、先生にすでにご処方いただいております抗認知症薬の継続処方をご検討いただけますと幸甚です。大変お忙しいところに、また誠に厚かましいお願いとなり恐縮ではございますが、どうか引き続きご高診賜りますようお願い申し上げます。"
            # （副作用のため切り替え）
            if shohou == "切り替え":
                kaishaku = f"神経心理検査では前回と比較して、ADAS曲線のnatural course（抗認知症薬を服用していない群）を上回っております。しかしながら{kirikae_riyu}、アリセプトの継続はリスクが高まると考え、例えばメマリーに切り替え、漸増処方することをご検討いただけますか（5→10→15→20mg）。大変差し出がましく、またお手数をおかけして申し訳ありませんが、どうか引き続きご高診賜りますようお願い申し上げます。"
        ...
        if hantei == False:
            if shohou_henkou == False:
                if shohou == "処方依頼なし":
                    kaishaku = f"神経心理検査では前回と比較して、ADAS曲線のnatural course(抗認知症薬を服用していない群）と有意差が認められませんが、ご本人ご家族と話し合い、今しばらくは{naifuku_genkou}の継続を希望されました。次回認知症精査まで同量を継続していく予定でおります。"
                if shohou == "継続":
                    kaishaku = f"神経心理検査では前回と比較して、ADAS曲線のnatural course(抗認知症薬を服用していない群）と有意差が認められませんが、ご本人ご家族と話し合い、今しばらくは{naifuku_genkou}の継続を希望されました。差し出がましく誠に恐縮ですが、抗認知症薬の継続処方をご検討賜れますと幸甚です。大変お忙しいところに、また誠に厚かましいお願いとなり恐縮ではございますが、どうか引き続きご高診賜りますようお願い申し上げます。"
            ...
            # （認知症薬増量処方依頼）
                    if shohou == "増量":
                        kaishaku = f"従いまして、画像上は経時的な認めませんでしたが、神経心理検査では前回と比較して悪化がみられます。ご本人ご家族の診察においても生活上の変化が報告され、抗認知症薬を増量する時期に来ていると愚考され、誠に厚かましいお願いとなり恐縮ではございますが、{naifuku_genkou}に増量することをご検討いただけますか。どうか引き続きご高診賜りますようお願い申し上げます。"
            # （認知症薬増量処方依頼）
                    if shohou == "切り替え":
                        kaishaku = f"従いまして、画像上は経時的な認めませんでしたが、神経心理検査では前回と比較して悪化がみられます。{kirikae_riyu}、アリセプトの継続はリスクが高まると考え、例えばメマリーに切り替え、漸増処方することをご検討いただけますか（5→10→15→20mg）。大変差し出がましく、またお手数をおかけして申し訳ありませんが、どうか引き続きご高診賜りますようお願い申し上げます。"
    ...
    if shohou_kaisi == True:
        if is_alicept != "":
            ...
            if zensoku == True:
                shohou_touin = f"{setumei_d}に、ご本人とご家族へ上記検査結果と抗認知症薬の効果（認知機能の低下の遅延、ADAS検査で効果を測定することなど）・副作用（コリン作用など）についてお伝えしました。ご本人とご家族が抗認知症薬の服薬を希望されましたので、気管支喘息の状況を考慮しつつ、アリセプト3mg1錠を処方させていただきました（アリセプトは気管支喘息において慎重投与になっております）。コリン作用に伴う食欲不振・下痢などの消化器症状の他、気管支狭窄による喘息発作などの副作用のモニタリングを行う予定です。"
            if shohou == "将来":
                shohou_irai = f"服薬が安定しましたところで、改めてご連絡させていただきます。その際には、アリセプト等の抗認知症薬の処方につきまして、先生からご処方をご検討賜れますと誠にありがたく存じます。大変お忙しいところに、また誠に厚かましいお願いとなり恐縮ではございますが、どうか引き続きご高診賜りますようお願い申し上げます。"
    if shohou == "初回依頼":
        shohou_irai = f"{naifuku_genkou}につきまして、食欲不振・下痢などの消化器症状の副作用もなく服薬は安定されております。今後につきましては、ご本人ご家族の希望もあり、先生から抗認知症薬のご処方をご検討賜れますと幸甚です。大変お忙しいところに、また誠に厚かましいお願いとなり恐縮ではございますが、どうか引き続きご高診賜りますようお願い申し上げます。"
    if shokai_kaisi == True:
        shohou_touin = ""
        shohou_irai = f"{setumei_d}に、ご本人とご家族へ上記検査結果と抗認知症薬の効果（認知機能の低下の遅延、ADAS検査で効果を測定することなど）・副作用（コリン作用など）についてお伝えしました。当院ではなく先生から抗認知症薬の開始をいただくことを希望されております。大変お忙しいところに、また誠に厚かましいお願いとなり恐縮ではございますが、アリセプト等の抗認知症薬の処方（たとえばアリセプトであれば血中の半減期が70-80時間であるため、副作用のチェックに3mg錠1T1Xで2-3週間、問題なければ5mg錠に増量など）についてご高診を賜れますと誠にありがたく存じます。"
        if zensoku_tain == True:
            shohou_touin = ""
            shohou_irai = f"{setumei_d}に、ご本人とご家族へ上記検査結果と抗認知症薬の効果（認知機能の低下の遅延、ADAS検査で効果を測定することなど）・副作用（コリン作用に伴う食欲不信や下痢などの消化器症状、および気管支喘息の悪化など）についてお伝えしました。当院ではなく先生から抗認知症薬の開始をいただくことを希望されております。大変お忙しいところに、また誠に厚かましいお願いとなり恐縮ではございますが、アリセプト等の抗認知症薬の処方（たとえばアリセプトであれば血中の半減期が70-80時間であるため、副作用のチェックに3mg錠1T1Xで2-3週間、問題なければ5mg錠に増量など）についてご高診を賜れますと誠にありがたく存じます。"
